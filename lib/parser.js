...
module.exports = {
	...
	parseInject: function () {
		this.inject = _.union(this.inject, match(this.$input('script').attr('inject'), /[\w\$]+/g));
	},
	...
	parseResolve: function () {
		var $ = this.$input;
		this.resolvers = $('script[resolve]').map(function() {
			var $script = $(this);
			return {
				inject: match($script.attr('inject'),  /[\w\$]+/g),
				params: $script.attr('params') || '',
				script: $script.html(),
				resolve: $script.attr('resolve'),
			};
		}).get();
	},
	...
	parseScript: function () {
		this.script = this.$input('script').not('[resolve]').html() || '';
	},
